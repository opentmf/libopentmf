...
#define LOAD_OPT_FUNC(x) (*api)->x = dlsym(driver, "opentmf_fg_" #x)

#define HANDLE_TYPE OPENTMF_HT_FUNCTIONGENERATOR
#define HANDLE_DATA ((struct fg_data*)handle->handle_data)
...
void fg_close(struct opentmf_handle* handle)
{
...
{
  handle->handle_data = calloc(1, sizeof(struct fg_data));
  if(!handle->handle_data)
    return OPENTMF_E_NO_MEMORY;

  handle->close = fg_close;
